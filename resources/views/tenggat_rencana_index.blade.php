    ...
    <div class="modal-content">
      <div class="modal-header">
        <div class="modal-title fw-semibold fs-5">
          {{ $tenggatRencana ? 'Edit' : 'Tambah' }} Tenggat {{ ucwords(str_replace('_', ' ',
          $kategoriTenggat->kategori_tenggat)) }}
        </div>
      ...
      <form action="{{ $action }}" method="POST" class="py-2">
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn btn-warning createJadwalAlert">Simpan</button>
        </div>
      </form>
    </div>
  </div>
...
<script>
  ...
  @foreach($kategoriTenggats as $kategoriTenggat)
    @if ($errors->has('tanggal_mulai') || $errors->has('jam_mulai') || $errors->has('tanggal_selesai') || $errors->has('jam_selesai'))
      @if (old('kategori_tenggat_id') == $kategoriTenggat->id)
        ...
        modal._element.addEventListener('shown.bs.modal', function () {
          ...
          var inputs = modalElement.querySelectorAll('input[name="tanggal_mulai"], input[name="jam_mulai"], input[name="tanggal_selesai"], input[name="jam_selesai"]');
