...
<div class="card mb-3 bg-white">
  <div class="card-body p-0 ">
    <div class="card-header p-3 fs-5 fw-bolder" style="background-color: #ececec;">Edit Kelompok <span class="fw-bolder tw-text-blue-500">{{ $kelompok->ketua->nama }}</span></div>
    <form action="/kelompok/{{ $kelompok->id }}" method="POST" class="px-4 py-2">
      @csrf
      ...
      <div class="form-group mt-2 mb-3">
        ...
        <select name="ketua_id" id="ketua_id" class="form-control placeholder-single" required>
          ...
          @foreach($listPegawai as $pegawai)
            <option value="{{ $pegawai->id }}" @selected(old('ketua_id') == $pegawai->id)>
