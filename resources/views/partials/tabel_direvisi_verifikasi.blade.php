<div class="table-responsive">
  <table class="table table-hover table-bordered mb-3 px-0 datatables" style="font-size: 0.7rem">
    ...
    <tbody>
      @foreach ($kelompokData['rencanaDirevisi'] as $index => $rencanaPembelajaran)
        <tr>
          {{-- NOMOR --}}
          ...
          <td class="px-2">
            ...
            <span
              class="badge {{ $rencanaPembelajaran->unitKerjaCanVerifying->status_revisi == 'belum_direvisi' ||
              $rencanaPembelajaran->unitKerjaCanVerifying->status_revisi == 'perlu_revisi_ulang'
                  ? 'text-bg-danger'
                  : ($rencanaPembelajaran->unitKerjaCanVerifying->status_revisi == 'sedang_direvisi'
                      ? 'text-bg-warning'
                      : 'text-bg-success') }} fs-1 p-1">
              {{ ucwords(str_replace('_', ' ', $rencanaPembelajaran->unitKerjaCanVerifying->status_revisi ?? '-')) }}
            </span>
          </td>

          {{-- AKSI --}}
          <td class="px-2">
            @if ($rencanaPembelajaran->unitKerjaCanVerifying->status_revisi === 'sudah_direvisi')
              <div class="text-center">
                <div class="btn-group" role="group">
                  ...
                  <a href="#" class="btn btn-warning btn-sm" title="Beri Revisi Tambahan" data-bs-toggle="modal"
                    ...
                    @if (!$isWithinDeadline) disabled @endif>
