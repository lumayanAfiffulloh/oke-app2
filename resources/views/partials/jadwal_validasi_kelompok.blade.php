    ...
    <div class="card-body p-4">
      <div class="d-flex justify-content-between align-items-center mb-3">
        <h5 class="fw-bolder m-0">
          <i class="ti ti-calendar-time me-2"></i>Jadwal Validasi
        </h5>
        <span class="badge bg-primary bg-opacity-10 text-primary">
          <i class="ti ti-user me-1"></i>Ketua Kelompok
        </span>
      </div>
      <hr class="mb-2">
      ...
      <div class="mb-3">
        ...
        <div class="alert alert-{{ $jadwalValidasi['sisaHari'] > 0 ? 'info' : 'danger' }} py-2 mb-0">
          ...
          {{ $jadwalValidasi['sisaHari'] > 0 ? 'Perencanaan belum dimulai' : 'Tenggat waktu telah berakhir!' }}
        </div>
        @endif
      </div>
      ...
      <div class="d-grid gap-0">
        @if ($jadwalValidasi['isActive'])
        <a href="{{ route('validasi_kelompok.index') }}" class="btn btn-success">
          <i class="ti ti-pencil me-2"></i> Validasi Rencana Sekarang
        </a>
        ...
        <button class="btn btn-dark btn-lg d-flex align-items-center justify-content-center" disabled>
          ...
          <span class="fs-3">{{ $jadwalValidasi['sisaHari'] > 0 ? 'Waktu validasi belum dimulai' : 'Masa Validasi Telah
            Berakhir' }}</span>
        </button>
        @endif
      </div>
