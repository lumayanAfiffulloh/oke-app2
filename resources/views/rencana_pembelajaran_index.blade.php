...
<div class="container-fluid px-3">
		...
		<div>
			...
			<div class="d-flex align-items-center gap-2">
				<span class="badge bg-primary bg-opacity-10 text-primary">
					<i class="ti ti-user me-1"></i> Suryani, S.E.
				</span>
				<span class="badge bg-secondary bg-opacity-10 text-secondary">
					<i class="ti ti-id me-1"></i> NIP. 123456789
				</span>
			</div>
		</div>
		<!-- Modified Alert with Deadline Logic -->
		<div class="d-flex align-items-center px-3 py-2 rounded"
			style="background-color: {{ $isWithinDeadline ? '#e8f5e9' : '#ffebee' }}; border-left: 4px solid {{ $isWithinDeadline ? '#4caf50' : '#f44336' }};">
			<i class="ti {{ $isWithinDeadline ? 'ti-calendar' : 'ti-alert-circle' }} me-2"
				style="color: {{ $isWithinDeadline ? '#4caf50' : '#f44336' }};"></i>
			<div>
				@if($isWithinDeadline)
				...
				<div style="font-size: 0.8rem; color: #2e7d32;">
					{{ Carbon\Carbon::parse($endDate)->isoFormat('dddd, D MMMM YYYY [pukul] HH:mm') }}
				</div>
				@else
				...
				<div style="font-size: 0.8rem; color: #c62828;">
					Batas akhir: {{ Carbon\Carbon::parse($endDate)->isoFormat('dddd, D MMMM YYYY [pukul] HH:mm') }}
				</div>
				@endif
			</div>
		</div>
	...
	<div class="card mb-4 pb-4 bg-white">
		<div class="card-body p-0">
			<div class="card-header px-3 pt-3 pb-2 fs-5 fw-bolder bg-white">
				<div class="d-flex justify-content-between align-items-center">
					<h5 class="mb-0">Daftar Rencana Pembelajaran</h5>
					<!-- Tombol Tambah -->
					@if($isWithinDeadline)
					...
					@else
					<button class="btn btn-outline-primary btn-sm ms-3 btn-disabled-custom" disabled>
						<span class="me-1">
							<i class="ti ti-clipboard-plus"></i>
						</span>
						<span>Tambah Rencana Pembelajaran</span>
					...
					<tbody>
						...
						<tr>
							...
							<td class="px-2">
								...
								@else
								<span class="fw-bold">*Rencana yang revisinya sudah dikirim tidak bisa dihapus atau diedit.</span>
								@endif
